...
import model.{StringLiteralExpression, BoolLiteralExpression, ClassDefinition}
...
class NoopBoolean(classDef: ClassDefinition, parameterInstances: Map[String, NoopObject],
  ...
  // TODO: this is an ugly way to make a new NoopBoolean
    "and" -> ((c: Context) => new BoolLiteralExpression(value && other(c)).evaluate(c)),
    "or" -> ((c: Context) => new BoolLiteralExpression(value || other(c)).evaluate(c)),
    "xor" -> ((c: Context) => new BoolLiteralExpression(value ^ other(c)).evaluate(c)),
    "not" -> ((c: Context) => new BoolLiteralExpression(! value).evaluate(c)),
